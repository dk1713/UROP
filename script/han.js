...
function initCarte(type) {
 ...
 var x = parseFloat(document.getElementById('xController').value);
    ...
    if (type === "#basis"){
        ...
        Plotly.newPlot("graph", computeAreaChange(x, y), layout);
    }
    return;
}
/**
...
function computeAreaChange(x, y) {
    currentPoint = [x, y];
    if (x*y > 0){
        if (x > 0){
        var a = 1 ;
    ...
    var data = [
        ...
        },
        {
            ...
            x : [x,x+a,x+a,x,x],
            y : [0,0,y,y,0],
            line: {simplify: false, color: orange},
            fill:'toself',
            opacity: 0.5
        },
        {
            ...
            x : [0,x,x,0,0],
            y : [y,y,y+b,y+b,y],
            line: {simplify: false, color: lilac},
            fill:'toself',
            opacity: 0.5
        },
        dominic.gObject()
...
function updatePlot() {
    ...
    } else if (href === "#combined"){
        data = computeAreaChange(x, y)
    }
